...
namespace Ct {
    ...
    private:
        ...
        bool Commute(const std::shared_ptr<Statement>& first, const std::shared_ptr<Expression>& second) {
        ...
        std::shared_ptr<Statement> DoStatement(const std::shared_ptr<StatementSequence>& statement) {
            return Sequence(DoStatement(statement->leftStatement_), DoStatement(statement->rightStatement_));
        }

        std::shared_ptr<Statement> DoStatement(const std::shared_ptr<Move>& moveStatement) {
        ...
        std::shared_ptr<Statement> DoStatement(const std::shared_ptr<Void>& voidStatement) {
        ...
        std::shared_ptr<Statement> DoStatement(const std::shared_ptr<Statement>& statement) {
        ...
        std::shared_ptr<Statement> ReorderStatement(const std::shared_ptr<Statement>& statement) {
        ...
        std::shared_ptr<ExpressionSequence> DoExpression(const std::shared_ptr<ExpressionSequence>& expressionSequence) {
            std::shared_ptr<Statement> statements = DoStatement(expressionSequence->statement_);
            std::shared_ptr<ExpressionSequence> expression = DoExpression(expressionSequence->expression_);
            return std::make_shared<ExpressionSequence>(Sequence(statements, expression->statement_), expression->expression_);
        }

        std::shared_ptr<ExpressionSequence> DoExpression(const std::shared_ptr<Expression>& expression) {
            auto expressionSequence = std::dynamic_pointer_cast<ExpressionSequence>(expression);
            if (expressionSequence != nullptr) {
        ...
        std::shared_ptr<ExpressionSequence> ReorderExpression(const std::shared_ptr<Expression>& expression) {
            std::shared_ptr<StatementExpressionList> list = Reorder(expression->Kids());
            return std::make_shared<ExpressionSequence>(list->Statement_, expression->Build(list->Expressions_));
        }

        std::shared_ptr<StatementExpressionList> Reorder(const std::vector<std::shared_ptr<Expression>>& list) {
          if (list.empty()) {
              return NopNull;
          }
          ...
          if (callExpression != nullptr) {
              Storage storage;
              std::shared_ptr<Expression> justExpression = std::make_shared<ExpressionSequence>(std::make_shared<Move>(std::make_shared<Temporary>(storage), head),
          ...
          std::shared_ptr<StatementExpressionList> statementExpressionList = Reorder(listSinceSecond);
          if (Commute(statementExpressionList->Statement_, expressionSequence->expression_)) {
              std::vector<std::shared_ptr<Expression>> newList{expressionSequence->expression_};
              std::copy(statementExpressionList->Expressions_.begin(), statementExpressionList->Expressions_.end(), std::back_inserter(newList));
              return std::make_shared<StatementExpressionList>(Sequence(expressionSequence->statement_, statementExpressionList->Statement_), newList);
          }


          Storage storage;
          std::vector<std::shared_ptr<Expression>> newList{std::make_shared<Temporary>(storage)};
          std::copy(statementExpressionList->Expressions_.begin(), statementExpressionList->Expressions_.end(), std::back_inserter(newList));
          return std::make_shared<StatementExpressionList>(
              Sequence(expressionSequence->statement_,
                  Sequence(std::make_shared<Move>(std::make_shared<Temporary>(storage), expressionSequence->expression_),
                      statementExpressionList->Statement_)),
              newList);
        }

        std::vector<std::shared_ptr<Statement>> Linearize(const std::shared_ptr<StatementSequence>& statement, const std::vector<std::shared_ptr<Statement>>& list) {
            return Linearize(statement->leftStatement_, Linearize(statement->rightStatement_, list));
        }

        std::vector<std::shared_ptr<Statement>> Linearize(const std::shared_ptr<Statement>& statement, const std::vector<std::shared_ptr<Statement>>& list) {
            auto statementSequence = std::dynamic_pointer_cast<StatementSequence>(statement);
            if (statementSequence != nullptr) {
        ...
        std::vector<std::shared_ptr<Statement>> Linearize(const std::shared_ptr<Statement>& statement) {
            return Linearize(DoStatement(statement), std::vector<std::shared_ptr<Statement>>());
        }

    private:
        const std::shared_ptr<StatementExpressionList> NopNull = std::make_shared<StatementExpressionList>(std::make_shared<Void>(std::make_shared<Constant>(0)), std::vector<std::shared_ptr<Expression>>());
