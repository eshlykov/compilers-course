...
namespace Irt {
    ...
    Translator::Translator() : codeFragment_{std::make_shared<Ct::CodeFragment>()} {
    ...
    void Translator::Visit(BinaryOperator* node) {
        ...
        std::optional<Ct::ArithmeticOperator> arithmeticOperator = ToCtArithmeticOperator(node->arithmeticOperator_);
        assert(arithmeticOperator.has_value());

        exp_ = std::make_shared<Ct::BinaryOperator>(arithmeticOperator.value(), leftExpression, rightExpression);
    }
    ...
    std::optional<Ct::ArithmeticOperator> Translator::ToCtArithmeticOperator(ArithmeticOperator arithmeticOperator) {
    ...
    std::optional<Ct::LogicalOperator> Translator::ToCtLogicalOperator(LogicalOperator logicalOperator) {
